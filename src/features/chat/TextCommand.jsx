...
const TextCommand = ({ input, setInput, handleSend, isListening, toggleSTT }) => {
  return (
    <GlassCard title="TEXT COMMAND" className="w-full lg:w-[44%] flex flex-col p-3 justify-center relative overflow-hidden">
      <div className="relative w-full flex items-center pt-0.5">
        ...
        />
        <button
          onClick={() => { playButtonClick(); toggleSTT && toggleSTT(); }}
        ...
        >
          <AudioLines size={11} className={isListening ? "animate-pulse text-cyan-200" : ""} />
        </button>
      </div>
    </GlassCard>
  );
